...
{
	x[0] = (x[0] + READ_ROUND_KEY_BYTE(wk[0])) & 0xFF; 
	x[2] = x[2] ^ READ_ROUND_KEY_BYTE(wk[1]); 
	x[4] = (x[4] + READ_ROUND_KEY_BYTE(wk[2])) & 0xFF; 
...
void EncryptRoundFunction(uint8_t *x, const uint8_t *sk)
{
	...
	x[7] = x[6];
	x[6] = (x[5] + (F1(x[4]) ^ READ_ROUND_KEY_BYTE(sk[2]))) & 0xFF;
	x[5] = x[4]; 
	x[4] = x[3] ^ ((F0(x[2]) + READ_ROUND_KEY_BYTE(sk[1])) & 0xFF);
	x[3] = x[2]; 
	x[2] = (x[1] + (F1(x[0]) ^ READ_ROUND_KEY_BYTE(sk[0]))) & 0xFF;
	x[1] = x[0]; 
	x[0] = temp7 ^ ((F0(temp6) + READ_ROUND_KEY_BYTE(sk[3])) & 0xFF);
}
...
void EncryptFinalTransfomation(uint8_t *x, const uint8_t *wk)
{
	...
	x[0] = (x[1] + READ_ROUND_KEY_BYTE(wk[4])) & 0xFF; 
	x[1] = x[2]; 
	x[2] = x[3] ^ READ_ROUND_KEY_BYTE(wk[5]); 
	x[3] = x[4];
	x[4] = (x[5] + READ_ROUND_KEY_BYTE(wk[6])) & 0xFF; 
	x[5] = x[6]; 
