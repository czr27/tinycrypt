...
void DecryptRoundFunction(uint8_t *x, const uint8_t *sk)
{
	...
	x[0] = x[1];
	x[1] = (x[2] - (F1(x[0]) ^ READ_ROUND_KEY_BYTE(sk[0]))) & 0xFF;
	x[2] = x[3];
	x[3] = x[4] ^ ((F0(x[2]) + READ_ROUND_KEY_BYTE(sk[1])) & 0xFF);	
	x[4] = x[5];
	x[5] = (x[6] - (F1(x[4]) ^ READ_ROUND_KEY_BYTE(sk[2]))) & 0xFF;
	x[6] = x[7];	
	x[7] = temp0 ^ ((F0(x[6]) + READ_ROUND_KEY_BYTE(sk[3])) & 0xFF);
}
...
void DecryptFinalTransfomation(uint8_t *x, const uint8_t *wk)
{
	...
	x[6] = x[5]; 
	x[5] = (x[4] - READ_ROUND_KEY_BYTE(wk[6])) & 0xFF;
	x[4] = x[3]; 
	x[3] = x[2] ^ READ_ROUND_KEY_BYTE(wk[5]);
	x[2] = x[1]; 
	x[1] = (x[0] - READ_ROUND_KEY_BYTE(wk[4])) & 0xFF;
	x[0] = temp;
}
