...
char *scrypt_pwd (uint8_t *pwd, size_t pwd_len,
  uint8_t *salt, size_t salt_len, uint64_t N, uint32_t r, uint32_t p)
{
  static char spwd[SCRYPT_HASH_LEN*2+1];
  uint8_t     result[SCRYPT_HASH_LEN*2];
  int         i;
  
  scrypt(pwd, pwd_len, salt, salt_len, N, r, p, result, sizeof(result));
  
  // convert to string
  for (i=0; i<SCRYPT_HASH_LEN; i++) {
    _snprintf (&spwd[i*2], 2, "%02x", result[i]);
  }
  return spwd;
}
...
int main (int argc, char *argv[])
{
  ...
  for (i=0; i<sizeof (pwds)/sizeof (char*); i++) {
    plen=strlen (pwds[i]);
    slen=strlen (salts[i]);
    
    printf ("\n  %s", 
      scrypt_pwd (pwds[i], plen, salts[i], 
        slen, n_val[i], r_val[i], p_val[i]));
  }    
  return 0;
}
