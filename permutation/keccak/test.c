...
void bin2hex(void *data, int len) {
    int i;
...
int main(void)
{
    ...
    keccak(state);
    //bin2hex(state, (25*WORDLEN)/8);
    equ = memcmp(state, tv1, sizeof(tv1))==0;
    printf("\nKeccak-f[%i, %i] test 1 %s\n", 
      25*WORDLEN,ROUNDS,equ ? "OK" : "FAILED");

    keccak(state);
    //bin2hex(state, (25*WORDLEN)/8);
    equ = memcmp(state, tv2, sizeof(tv2))==0;
    printf("Keccak-f[%i, %i] test 2 %s\n", 
      25*WORDLEN,ROUNDS, equ ? "OK" : "FAILED");
