...
CASE1:			
      ldi					            ; first byte is always copied as literal
;
;  main decompressor loop

MainLoop:		
      GET_BIT 
      jr c,CASE1			        ; "1"+BYTE = copy literal
			push de 
      ld d,#FF
			ld bc,2
			GET_BIT 
...
ActualCopy		
      ex (sp),hl				      ; HL = dest, (SP) = src
			ex de,hl 
      add hl,de			          ; DE = dest, HL = dest-offset
			ldir
			pop hl 
      jr MainLoop
...
ReadLength		
      GET_BIT 
      rl c
      jr c,WayOut		          ; overflow signals the end of the data
			djnz ReadLength
			inc c
